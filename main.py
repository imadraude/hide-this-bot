...
from models import Post, PostMode, User
...
def create_post(author: User, content: str, scope: set = ()):
    try:
        ...
        logger.error(
            "new post cannot be created by " + get_formatted_username_or_id(author)
        )
        logger.error(e)
        return
...
async def inline_query_hide(inline_query: types.InlineQuery):
    try:
        ...
        if len(scope) > 1:
            formatted_scope += (
                " %s " % locales[target.language_code].and_connector + scope[-1]
            )
        else:
...
async def inline_query_spoiler(inline_query: types.InlineQuery):
    try:
        ...
        await inline_query.answer(
            [rsc.query_results.mode_spoiler(target.language_code, post.get_id(), body)]
        )
    except Exception as e:
        logger.error(e)
        logger.warning(
        ...
        if post.can_be_accessed_by(call.from_user, PostMode[mode]):
            ...
            await call.answer(
                post.content.replace(
                    "{username}", get_formatted_username_or_id(call.from_user)
                )
                .replace("{uid}", "id" + str(target.user_id))
                .replace(
                    "{lang}",
                    "unknown" if target.language_code is None else target.language_code,
                )
                .replace("{pid}", "#" + post_id)
                ...
                .replace(
                    "{last_name}", "" if target.last_name is None else target.last_name
                ),
                True,
            )
        else:
